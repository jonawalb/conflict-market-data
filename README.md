...
- **`escalation` is a candidate flag, not a validated panel.** It is regex over question and event title, tested against 25 cases including real false positives found live ("Warnock" matching `war`; airline strikes and Counter-Strike matches matching `strike`). Hand-validate before analysis and report precision.
